...
class OptimalQuantityFilters(BaseModel):
    ...
    item_code: str = Field(None, description="Item code analyzed")
    item_name: str = Field(None, description="Item name analyzed")
    horizon_days: int = Field(default=30, description="Forecast horizon in days")
    lookback_days: int = Field(
        default=30, description="Historical data lookback period"
    )
    calculation_method: str = Field(
        default="mean", description="Calculation method used"
    )
...
class BestSupplierFilters(BaseModel):
    ...
    item_code: str | None = Field(None, description="Item code to purchase")
    item_name: str | None = Field(None, description="Item name to purchase")
...
class BestSupplierOutput(MCPToolOutputSchema):
    ...
    items: list[SupplierOption] | None = Field(
...
class POInputItem(BaseModel):
    """Schema for input item in create_consolidated_po."""

    item_code: str = Field(
        ..., description="ERPNext item code (or item name for fuzzy search)"
    )
    qty: int = Field(..., description="Quantity to order")
    warehouse: str | None = Field(
        None, description="Target warehouse (optional, uses default if not provided)"
    )
    rate: float | None = Field(
        None,
        description="Unit rate (optional, looks up from supplier history if not provided)",
    )


class POLineItem(BaseModel):
    """Schema for individual line item in PO output."""
...
class CreatedPO(BaseModel):
    """Schema for item in purchase_orders list - item details only."""
...
class ConsolidatedPOFilters(BaseModel):
    ...
    items: str = Field(..., description="JSON array of items requested")
...
class ConsolidatedPOSummary(BaseModel):
    ...
    url: str = Field(..., description="Direct URL to the Purchase Order")
...
class ConsolidatedPOOutput(MCPToolOutputSchema):
    ...
    items: list[CreatedPO] | None = Field(None, description="List of ordered items")
...
class PriceVarianceItem(BaseModel):
    """Schema for individual price history item - for items list."""
...
class PriceVarianceFilters(BaseModel):
    ...
    item_code: str | None = Field(None, description="Item code analyzed")
    item_name: str | None = Field(None, description="Item name analyzed")
...
class PriceVarianceSummary(BaseModel):
    ...
    item_code: str = Field(..., description="ERPNext item code")
    item_name: str = Field(..., description="Item name")
    current_price: float = Field(..., description="Current/proposed price (latest)")
    ...
    variance_pct: float = Field(..., description="Percentage difference from average")
    ...
    recommendation: str = Field(..., description="Detailed recommendation message")


class PriceVarianceOutput(MCPToolOutputSchema):
    """Schema for monitor_price_variance tool output."""

    items: list[PriceVarianceItem] | None = Field(
        None, description="Price history records"
    )
