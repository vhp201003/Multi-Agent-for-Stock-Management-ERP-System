...
class InventoryMCPServer(BaseMCPServer):
    ...
    async def _register_tools(self) -> None:
        """Register comprehensive inventory management tools."""

        @self.mcp.tool(
            name="check_stock",
            description="Check current stock level for a specific product. Returns detailed availability status with reservations.",
            annotations={
                "category": "inventory",
                "icon": "📦",
        ...
        @self.mcp.tool(
            ...
            description="Retrieve stock movement history for analysis. Supports filtering by product, date range, and movement type.",
            annotations={
                "category": "inventory",
                "icon": "📊",
                "security_level": "validated",
            ...
            try:
                ...
                for record in self._mock_history_data:
                    ...
                    if (
                        ...
                        and record["item_code"] != validated_product_id
                    ):
                        continue
                ...
                total_in = sum(
                    1 for r in filtered_history if r["movement_type"] == "IN"
                )
                total_out = sum(
                    1 for r in filtered_history if r["movement_type"] == "OUT"
                )
                total_value = sum(
        ...
        @self.mcp.tool(
            ...
            description="Query inventory data with advanced filtering. Supports category, stock level, and supplier filters.",
            annotations={
                "category": "inventory",
                "icon": "🔍",
                "security_level": "validated",
        ...
        async def query_inventory_data(
            ...
            try:
                ...
                for product_id, item_data in self._mock_inventory_data.items():
                    ...
                    if (
                        supplier
                        and supplier.lower() not in item_data["supplier"].lower()
                    ):
                        continue
                ...
                reorder_needed = sum(
                    1 for item in limited_items if item["needs_reorder"]
                )
        ...
        @self.mcp.tool(
            name="monitor_stock_availability",
            description="Monitor stock availability across all products. Identifies items requiring attention with detailed alerts.",
            annotations={
                "category": "inventory",
                "icon": "⚠️",
                "security_level": "validated",
                ...
                for product_id, item_data in self._mock_inventory_data.items():
                    ...
                    if alert_level != "all":
                        if (
                            ...
                            alert_level == "reorder"
                            and "REORDER_NEEDED" not in alert_types
                        ):
                            continue
    ...
    async def _register_resources(self) -> None:
        """Register inventory data resources."""

        @self.mcp.resource(
            uri="stock://levels",
            name="Stock Levels Dashboard",
            description="Comprehensive dashboard of current stock levels with analytics and alerts",
        ...
        async def get_stock_levels() -> str:
            ...
            try:
                ...
                raise RuntimeError(
                    f"Unable to generate stock levels dashboard: {str(e)}"
                )

        @self.mcp.resource(
            uri="stock://alerts",
            name="Stock Alerts Feed",
            description="Real-time feed of stock alerts and notifications requiring immediate attention",
            mime_type="application/json",
        )
            ...
            try:
                ...
                for product_id, item_data in self._mock_inventory_data.items():
                    ...
                    if available_stock <= 0:
                        ...
                        available_stock
                        <= self.inventory_config.critical_stock_threshold
                    ):
                        critical_alerts.append(
                            {
                                "type": "CRITICAL_STOCK",
